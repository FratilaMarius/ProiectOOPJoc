...
{ // when entering a new room we give it exits and a texture
...
{ // we only call this one in the constructor
...
int Labyrinth::Move(const std::string &where)
{
  ...
  case 'u':
    if (playerCords[0] - 1 == -1)
    { // check for out of bounds
      std::cout << "\nWall\n";
      shouldDisplayDeadEndText = 1;
      break;
    }
    if (!layout[playerCords[0]][playerCords[1]].SeeIfExitHere("up"))
  ...
  case 'd':
    if (playerCords[0] + 1 == width)
    { // check for out of bounds
      std::cout << "\nWall\n";
      shouldDisplayDeadEndText = 1;
      break;
    }
    if (!layout[playerCords[0]][playerCords[1]].SeeIfExitHere("down"))
    { // if we cant go that way
      std::cout << "\nDead end\n";
      shouldDisplayDeadEndText = 1;
      break;
  ...
  case 'l':
    if (playerCords[1] - 1 == -1)
    { // check for out of bounds
      std::cout << "\nWall\n";
      shouldDisplayDeadEndText = 1;
      break;
    }
    if (!layout[playerCords[0]][playerCords[1]].SeeIfExitHere("left"))
    { // if we cant go that way
      std::cout << "\nDead end\n";
      shouldDisplayDeadEndText = 1;
      break;
  ...
  case 'r':
    if (playerCords[1] + 1 == height)
    { // check for out of bounds
      std::cout << "\nWall\n";
      shouldDisplayDeadEndText = 1;
      break;
    }
    if (!layout[playerCords[0]][playerCords[1]].SeeIfExitHere("right"))
    { // if we cant go that way
      std::cout << "\nDead end\n";
      shouldDisplayDeadEndText = 1;
      break;
  ...
  if (newX == -1 && newY == -1)
    throw(LabExceptionCouldntMove(playerCords[0], playerCords[1]));
  ...
  if (moves > 20 && chanceForExit > 10)
  { // after 10 moves we start to get close to the exit
    chanceForExit -= 2;
  }
  if (chanceForExit <= 10 && RNG() % 5 == 0)
    finish = 1;
